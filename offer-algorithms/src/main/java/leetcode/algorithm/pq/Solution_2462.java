...
//                1, 3
        ...
        System.out.println(sol.totalCostBetter(costs,
                //24,8
                11, 2
//                1, 3
        ));
    ...
    /**
     * [first][mid][last]-> 题目要求在first, 和last 中选择k个最小元素,  如果 k个元素都分布在mid 区域,
     *
     * @param costs
     * @param k
     * @param candidates
    ...
    public long totalCostBetter(int[] costs, int k, int candidates) {
        ...
        while (k-- > 0 && l <=r) {
            if (pqf.peek() <= pql.peek()) {
                res += pqf.peek();
                System.out.println("first pick:" + pqf.peek());
                pqf.poll();
                pqf.add(costs[l++]);

            } else {
                res += pql.peek();
                System.out.println("last pick: " + pql.peek());
                pql.poll();
                pql.add(costs[r--]);
            }
        }
