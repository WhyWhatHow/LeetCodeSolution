...
public class Solution_2081 {
    ...
    public long kMirror(int k, int n) {
        ...
        for (int base = 1;; base *= 10) {
            ...
            // odd gen , example: base =10 , res 101, 999
            for (int i = base; i < base * 10; i++) {
                long x = i;
                for (int t = i / 10; t > 0; t /= 10) // prefix 122, -> 12221
                    x = x * 10 + t % 10;
                if (check(x, k)) {
                    res += x;
                    cnt++;
            ...
            // even gen example: base=10 , res [1001, 9999]
            for (int i = base; i < base * 10; i++) {
                long x = i;
                for (int t = i; t > 0; t /= 10) { // example t= 12, -> 1221
                    x = x * 10 + t % 10;
                }
                if (check(x, k)) {
                    res += x;
    ...
    boolean check(long x, int k) {
        var s = Long.toString(x, k);
